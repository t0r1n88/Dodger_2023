...
def check_employers_target(df:pd.DataFrame,name_file:str,name_sheet:str):
    ...
    df['Результат'] = df[['8', '9']].apply(lambda x: 'Правильно' if (x[0] in lst_emp_eight and x[1] in lst_emp_nine) or (x[0] in lst_work_eight and x[1] in lst_work_nine) or (x[0] in lst_all_eight and x[1] in lst_all_nine) else 'Неправильно', axis=1)
