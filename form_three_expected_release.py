...
def prepare_form_three_employment(path_folder_data:str,path_to_end_folder):
    ...
    try:
        for file in os.listdir(path_folder_data):
            ...
            if not file.startswith('~$') and file.endswith('.xlsx'):
                ...
                if 'Форма 3 ожидаемый выпуск' not in lst_temp_sheets:  # проверяем наличие листа с названием в файле
                    ...
                    error_df = pd.concat([error_df, temp_error_df], axis=0, ignore_index=True)
                    continue
                df = pd.read_excel(f'{path_folder_data}/{file}', skiprows=5, sheet_name='Форма 3 ожидаемый выпуск',dtype=str)
        ...
        messagebox.showerror('Кассандра Подсчет данных по трудоустройству выпускников',
                             ...
                             f'Проверьте файл на соответствие шаблону Формы 3 (Ожидаемый выпуск)')
    else:
