# -*- coding: utf-8 -*-
"""
Функции для проверки данных нозологий, поскольку форма сбора изменилась а для 5 строк нет
"""
...
def check_first_error_nose(df: pd.DataFrame, name_file, tup_correct):
    """
...
def check_second_error_nose(df: pd.DataFrame, name_file, tup_correct):
    """
...
def check_third_error_nose(df: pd.DataFrame, name_file, border, tup_correct):
    """
    Функция для проверки правильности введеденных данных
...
def check_fourth_error_nose(df: pd.DataFrame, name_file, border, tup_correct):
    """
    Функция для проверки правильности введеденных данных
...
def check_fifth_error_nose(df: pd.DataFrame, name_file, border, tup_correct):
    """
    Функция для проверки правильности введеденных данных
...
def check_sixth_error_nose(df: pd.DataFrame, name_file, border, tup_correct: tuple):
    """
    Функция для проверки правильности введеденных данных
...
def check_seventh_error_nose(df: pd.DataFrame, name_file, border, tup_correct: tuple):
    """
    Функция для проверки правильности введеденных данных
...
def check_error_nose(df: pd.DataFrame, name_file, tup_correct: tuple):
    ...
    for i in range(1, quantity + 1):
        ...
        first_error_df = check_first_error_nose(temp_df.copy(), name_file, tup_correct)
        ...
        second_error_df = check_second_error_nose(temp_df.copy(), name_file, tup_correct)
        # добавляем результат проверки в датафрейм
        error_df = pd.concat([error_df, second_error_df], axis=0, ignore_index=True)

        # Проводим проверку стр. 06 = стр. 02 + стр. 04
        third_error_df = check_third_error_nose(temp_df.copy(), name_file, border, tup_correct)
        # добавляем результат проверки в датафрейм
        ...
        fourth_error_df = check_fourth_error_nose(temp_df.copy(), name_file, border, tup_correct)
        # добавляем результат проверки в датафрейм
        ...
        fifth_error_df = check_fifth_error_nose(temp_df.copy(), name_file, border, tup_correct)
        # добавляем результат проверки в датафрейм
        error_df = pd.concat([error_df, fifth_error_df], axis=0, ignore_index=True)

        # Проводим проверку стр.03 <= стр.02
        sixth_error_df = check_sixth_error_nose(temp_df.copy(), name_file, border, tup_correct)
        error_df = pd.concat([error_df, sixth_error_df], axis=0, ignore_index=True)

        # Проводим проверку стр.02 и стр.04 и стр.05 < стр.01
        seventh_error_df = check_seventh_error_nose(temp_df.copy(), name_file, border, tup_correct)
...
def create_check_tables_nose(high_level_dct: dict):
    """
