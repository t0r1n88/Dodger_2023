...
# для обработки региональных мониторингов
from reg_temp_monitoring_september_2026 import prepare_september_2025 # мониторинг сентябрь 2025
...
"""
Функции для построения временных рядов по листам
"""

def select_folder_data_list_vacance():
    """
    Функция для выбора папки cодержащей своды
    :return:
    """
    global path_folder_data_list_vacance
    path_folder_data_list_vacance = filedialog.askdirectory()


def select_file_list_vacance():
    """
    Функция для выбора файла с организациями
    """
    global file_list_vacance
    # Получаем путь к файлу
    file_list_vacance = filedialog.askopenfilename(filetypes=(('Excel files', '*.xlsx'), ('all files', '*.*')))


def select_file_params_cols():
    """
    Функция для выбора файла с организациями
    """
    global file_params_cols
    # Получаем путь к файлу
    file_params_cols = filedialog.askopenfilename(filetypes=(('Excel files', '*.xlsx'), ('all files', '*.*')))





def select_end_folder_list_vacance():
    """
    Функция для выбора конечной папки куда будут складываться итоговые файлы
    :return:
    """
    global path_to_end_folder_list_vacance
    path_to_end_folder_list_vacance = filedialog.askdirectory()


def processing_list_vacance():
    """
    Фугкция для обработки данных мониторинга 5 строк базовый мониторинг
    :return: файлы Excel  с результатами обработки
    """
    try:
        vac_name_sheet = entry_sheet_name_list_vac.get()
        processing_time_series_list_vac(path_folder_data_list_vacance, path_to_end_folder_list_vacance,file_params_cols,file_list_vacance,vac_name_sheet)
...
if __name__ == '__main__':
    window = Tk()
    window.title('Кассандра Внутренняя версия ЦОПП РБ 1.0')
    # Устанавливаем размер и положение окна
    ...
    global file_list_vacance
    file_list_vacance = 'Не выбрано' # костыль
    ...
    """
    Вкладка создания временных рядов по сводам
    """
    ...
    """
        Вкладка создания временных рядов по спискам вакансий
        """

    tab_extract_list = ttk.Frame(tab_control)
    tab_control.add(tab_extract_list, text='Создание временных рядов\nпо спискам вакансий')

    extract_list_frame_description = LabelFrame(tab_extract_list)
    extract_list_frame_description.pack()

    lbl_hello_extract_list = Label(extract_list_frame_description,
                                   text='Центр опережающей профессиональной подготовки Республики Бурятия\n'
                                        'Построение временных рядов по основным показателям\n'
                                        'на основе файлов СПИСКОВ вакансий созданных функцией Свод Работа в России',
                                   width=60)
    lbl_hello_extract_list.pack(side=LEFT, anchor=N, ipadx=25, ipady=10)

    # Картинка
    path_to_img_extract_list = resource_path('logo.png')
    img_path_to_img_extract_list = PhotoImage(file=path_to_img_extract_list)
    Label(extract_list_frame_description,
          image=img_path_to_img_extract_list, padx=10, pady=10
          ).pack(side=LEFT, anchor=E, ipadx=5, ipady=5)

    # Создаем область для того чтобы поместить туда подготовительные кнопки(выбрать файл,выбрать папку и т.п.)
    frame_data_extract_list = LabelFrame(tab_extract_list, text='Подготовка')
    frame_data_extract_list.pack(padx=10, pady=10)

    btn_choose_data_extract_list = Button(frame_data_extract_list,
                                          text='1) Выберите папку с данными', font=('Arial Bold', 20),
                                          command=select_folder_data_list_vacance
                                          )
    btn_choose_data_extract_list.pack(padx=10, pady=10)


    btn_choose_params_cols = Button(frame_data_extract_list,
                                            text='2) Выберите файл с колонками',
                                            font=('Arial Bold', 20),
                                            command=select_file_params_cols
                                            )
    btn_choose_params_cols.pack(padx=10, pady=10)


    # Определяем текстовую переменную
    entry_sheet_name_list_vac = StringVar()
    # Описание поля
    label_sheet_name_list_vac = Label(frame_data_extract_list,
                                             text='3) Введите название листа, из которого собираются вакансии')
    label_sheet_name_list_vac.pack(padx=10, pady=10)
    # поле ввода имени листа
    first_sheet_name_entry_list_vac = Entry(frame_data_extract_list, textvariable=entry_sheet_name_list_vac,
                                             width=30)
    first_sheet_name_entry_list_vac.pack(padx=10, pady=10)





    btn_choose_filter_extract_list = Button(frame_data_extract_list,
                                            text='Необязательная опция\n Выберите файл с отслеживаемыми вакансиями',
                                            font=('Arial Bold', 10),
                                            command=select_file_list_vacance
                                            )
    btn_choose_filter_extract_list.pack(padx=10, pady=10)

    # Создаем кнопку для выбора папки куда будут генерироваться файлы

    btn_choose_end_folder_extract_list = Button(frame_data_extract_list,
                                                text='4) Выберите конечную папку', font=('Arial Bold', 20),
                                                command=select_end_folder_list_vacance
                                                )
    btn_choose_end_folder_extract_list.pack(padx=10, pady=10)

    btn_proccessing_data_extract_list = Button(tab_extract_list, text='5) Обработать данные',
                                               font=('Arial Bold', 20),
                                               command=processing_list_vacance
                                               )
    btn_proccessing_data_extract_list.pack(padx=10, pady=10)
    ...
    lbl_about = Label(about_frame_description,
                      text="""Кассандра - Программа для обработки выгрузок по вакансиям\n
                       с портала Работа в России (trudvsem.ru).

                           Версия 7.1
                           Язык программирования - Python 3\n
                           Используемая лицензия BSD-2-Clause\n
                           Copyright (c) <2026> <Будаев Олег Тимурович>
                           Адрес сайта программы: https://itdarhan.ru/cassandra.html\n
                           Свидетельство о государственной регистрации № 2026665008
                           \n
