...
def check_cross_first_error_df(df1:pd.DataFrame,df2:pd.DataFrame, name_file):
    ...
    for row in check_df_now.itertuples():
        temp_error_df = pd.DataFrame(data=[[f'{name_file}', f'Перекрестная проверка трудоустроен/будет трудоустроен: {row[2]} не совпадают данные !!! по форме 1 для этой специальности трудоустроено {row[4]} чел.'
                                                            f' в форме 2 по этой специальности найдено {int(row[1])} чел.',
...
def check_cross_second_error(df1:pd.DataFrame,df2:pd.DataFrame, name_file):
  ...
  # создаем датафрейм, по колонке 06 заключили договор о целевом обучении и уже трудоустроенные
    ...
    for row in row_with_nan.itertuples():
        temp_error_df = pd.DataFrame(data=[[f'{name_file}',
                                            f'Перекрестная проверка целевиков (трудоустроен/будет трудоустроен): в форме 1 нйадено {row[1]} а в форме 2 {row[3]} не совпадают данные !!! Отсутствуют данные по этой специальности либо в форме 1 либо в форме 2',
    ...
    for row in row_with_nan.itertuples():
        temp_error_df = pd.DataFrame(data=[[f'{name_file}',
                                            ...
                                            'В форме 1 для специальности указаны выпускники целевики которые БУДУТ трудоустроены, но в форме 2 такой специальности не найдено или наоборот. ДАННЫЕ ФАЙЛА НЕ ОБРАБОТАНЫ !!!']],
                                     columns=['Название файла', 'Строка или колонка с ошибкой',
                                              'Описание ошибки'])
        error_df = pd.concat([error_df, temp_error_df], axis=0, ignore_index=True)
    # # отбираем все строки где нет нан
...
for file in os.listdir(path_folder_data):
    ...
    if not file.startswith('~$') and file.endswith('.xlsx'):
        ...
        if not check_two_rows_spec:
            temp_error_df = pd.DataFrame(data=[[f'{name_file}', '',
                                                'Проверьте правильность заполнения колонки 04. Для каждой спец./проф. должны быть  только строки 01 и 02 не считая строки с проверкой. Проверьте наличие ПУСТОЙ строки после таблицы! Последняя строка проверки должна быть заполнена полностью! ДАННЫЕ ФАЙЛА НЕ ОБРАБОТАНЫ !!! ']],
                                         columns=['Название файла', 'Строка или колонка с ошибкой',
                                                  'Описание ошибки'])
            error_df = pd.concat([error_df, temp_error_df], axis=0, ignore_index=True)
            continue
...
opk_finish_df = finish_df[['Код специальности','Номер строки','Наименование показателей (категория выпускников)','Суммарный выпуск 2023 г.','Трудоустроены на предприятия оборонно-промышленного комплекса*',
                           'Будут трудоустроены на предприятия оборонно-промышленного комплекса* ']]

opk_finish_df.to_excel(f'{path_to_end_folder}/Трудоустройство по отраслям (ОПК) от {current_time}.xlsx', index=False)
finish_df.to_excel(f'{path_to_end_folder}/Полная таблица Трудоустройство по отраслям от {current_time}.xlsx', index=False)
# обрабатываем список из второй формы
all_form2.columns = ['Субъект Российской Федерации','Код и наименование профессии, специальности ','Наименование образовательной организации среднего профессионального образования, в которой обучался выпускник',
                     ...
                     'ИНН предприятия','Наименование предприятия/филиала предприятия/структурного подразделения предприятия оборонно-промышленного комплекса, на котором трудоустроен выпускник/планирует трудоустройство',
                     'Наименование профессии/должности, по которой трудоустраивается (планирует трудоустройство) выпускник',
                     'Трудоустройство в соответствии с освоенной профессией, специальностью (да/нет)','Сложности при трудоустройстве (например, при взаимодействии с предприятием, наличие рисков расторжения договора о целевом обучении и т.д.), реализуемые меры (описательная часть. В отсутствие сложностей - пропустите графу)']
all_form2.to_excel(f'{path_to_end_folder}/Общий список по форме 2 от  {current_time}.xlsx', index=False)
