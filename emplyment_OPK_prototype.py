...
path_folder_data_opk = 'data/ОПК Воронеж'
path_to_end_folder_opk = 'data'
...
for file in os.listdir(path_folder_data_opk):
    ...
    if not file.startswith('~$') and file.endswith('.xlsx'):
        ...
        temp_wb = openpyxl.load_workbook(f'{path_folder_data_opk}/{file}', read_only=True) # открываем файл в режиме чтения
...
opk_finish_df.to_excel(f'{path_to_end_folder_opk}/Трудоустройство по специальностям (ОПК) от {current_time}.xlsx', index=False)
finish_df.to_excel(f'{path_to_end_folder_opk}/Полная таблица Трудоустройство по отраслям от {current_time}.xlsx', index=False)
...
with pd.ExcelWriter(f'{path_to_end_folder_opk}/Итоги по ОПК,целевикам и всем колонкам {current_time}.xlsx', engine='openpyxl') as writer:
...
all_form2.to_excel(f'{path_to_end_folder_opk}/Общий список по форме 2 от  {current_time}.xlsx', index=False)
